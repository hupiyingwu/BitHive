...
f:
        ...
        Debug.Print "正在请求项目"
            ...
            If user = username Then
                If scripthash = "" Then scripthash = InputBox("result")
                scriptname = pro
                mypeaple = peaple
            End If
            Dim stmp As Single
    ...
    key = "script-return"
            If checkstr(command, key, head) = True Then
                ...
                scriptr = scriptr + creatdata(address, h)
        ...
        If checkstr(command, key, head) = True Then
            ...
            While sp(scriptu, k) <> "null"
                If readata(scriptr, sp(scriptu, k)) <> hexing(Hash(sp(scriptu, k) + bar)) Then GoTo sus
                k = k + 1
            Wend
            sys = sys - 10
            m = m + 10
...
incommand.Text = "script-excuse " + InputBox("input its address") + ","
incommand.Text = incommand.Text + InputBox("input name") + ","
incommand.Text = incommand.Text + InputBox("input total number of codes") + ","
incommand.Text = incommand.Text + InputBox("input commission") + ","
incommand.Text = incommand.Text + InputBox("input keywords")
End Sub

Private Sub sa4_Click()
incommand.Text = "file-write " + InputBox("file name") + "," + InputBox("text")
...
address = InputBox("input its address")
money = InputBox("input money")
incommand.Text = "wallet-pay " + address + "," + money + "," + InputBox("input notes")
MsgBox "command is created"
...
Dim numkey As String
numkey = "430610"
If dignum > 4 Then numkey = "43"
While InStr(hexing(Hash(getAddress(username) + myblock & Int(lucknum))), numkey) <= 0
    doing.Caption = "trying" & lucknum
    DoEvents
    lucknum = lucknum + 1
Wend
dignum = dignum + 1
